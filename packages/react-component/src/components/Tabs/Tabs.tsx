...
export const Tabs = ({ titles, handleSelectTab, variant = 'hat' }: TabsProps) => {
  ...
  if (variant === 'hat') {
    return (
      <ul className={css({display:'flex', alignItems:'center', borderBottom: `1px solid ${theme.color.gray.darker}`})}>
        {titles.map((title) => (
          <HatTab
            key={title}
            title={title}
      ...
      {titles.map((title, index, array) => (
        <div key={title} className={css({display:'flex', alignItems:'center'})}>
          <RectTab
            ...
            setSelectedTab={() => setSelectedTab(title)}
          />
          {index !== array.length - 1 && <div className={css({height:'16px', borderLeft: `1px solid ${theme.color.gray.darker}`})} />}
        </div>
      ))}
